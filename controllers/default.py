...
def pygal():
    ...
    rows=db.executesql("SELECT d.nombre,sum(rd.areaafect) FROM rodald rd, plan p, seccionjudicial sj, departamento d WHERE rd.plan=p.id AND p.sjudicial=sj.id AND sj.departamento=d.id AND p.cby=%i GROUP BY d.nombre ORDER BY d.nombre" % auth.user_id)
